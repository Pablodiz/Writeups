...
def string_to_blockchain(string:str):
    blocks = string.split('-')
    blockchain = []
    for block_hash in blocks:
        block = Block(0, "", 0, block_hash, 0)
...
def decrypt(ciphertext, key):
    ...
    unpaded_plaintext = unpad(plaintext)
    return str(unpaded_plaintext)
...
def main():
    ...
    decrypted_blockchain_string = decrypt(encrypted_blockchain, key)
    
    decrypted_blockchain = string_to_blockchain(decrypted_blockchain_string)
    
    for block in decrypted_blockchain:
        print("Block Hash:", block.encoded_transactions)
